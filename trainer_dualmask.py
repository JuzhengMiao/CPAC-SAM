...
def trainer_acdc_dualmask_prompt_ssl_fixcoe_up_labelguide_PCR2_3_PCR(args, model, snapshot_path, multimask_output, low_res):
    from datasets.dataset_ACDC import ACDC_dataset, RandomGenerator
    logging.basicConfig(filename=snapshot_path + "/log.txt", level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs_round2 = model(image_batch, multimask_output, args.img_size, 1, args.promptmode, label_batch[:labeled_bs], args.scaling) 
            outputs_round2_1 = outputs_round2['low_res_logits1']
            outputs_round2_1_r = outputs_round2['low_res_logits1_2']
            outputs_round2_2 = outputs_round2['low_res_logits2']
            outputs_round2_soft1 = torch.softmax(outputs_round2_1, dim=1)
            outputs_round2_soft1_r = torch.softmax(outputs_round2_1_r, dim=1)
            outputs_round2_soft2 = torch.softmax(outputs_round2_2, dim=1)
            ...
            if iter_num < args.warm_iter:
                ...
                consistency_loss2 = 0.5*(ce_loss(outputs_round2_2[labeled_bs:], pseudo_outputs1.long()) + dice_loss(outputs_round2_2[labeled_bs:], pseudo_outputs1, softmax=True))
                consistency_loss1_r = 0.5*(ce_loss(outputs_round2_1_r[labeled_bs:], pseudo_outputs1.long()) + dice_loss(outputs_round2_1_r[labeled_bs:], pseudo_outputs1, softmax=True)) + 0.5*(ce_loss(outputs_round2_1[labeled_bs:], pseudo_outputs1.long()) + dice_loss(outputs_round2_1[labeled_bs:], pseudo_outputs1, softmax=True))

                del pseudo_outputs1
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()

            loss2 = 0.5*(supervised_round2_loss1 + supervised_round2_loss1_r) + consistency_weight * consistency_loss2 + args.coe_PCR * consistency_loss1_r
            optimizer.zero_grad()
            loss2.backward()
            optimizer.step()

            del outputs_round2_1, outputs_round2_1_r, outputs_round2_2, outputs_round2_soft1, outputs_round2_soft1_r, outputs_round2_soft2
            if torch.cuda.is_available():
                torch.cuda.empty_cache()

            outputs_round3 = model(image_batch, multimask_output, args.img_size, 0, args.promptmode, label_batch[:labeled_bs], args.scaling) 
            outputs_round3_1 = outputs_round3['low_res_logits1']
            outputs_round3_2 = outputs_round3['low_res_logits2']
            outputs_round3_2_r = outputs_round3['low_res_logits2_2']
            outputs_round3_soft1 = torch.softmax(outputs_round3_1, dim=1)
            outputs_round3_soft2 = torch.softmax(outputs_round3_2, dim=1)
            outputs_round3_soft2_r = torch.softmax(outputs_round3_2_r, dim=1)
            ...
            if iter_num < args.warm_iter:
                ...
                pseudo_outputs2 = torch.argmax(outputs_round3_soft2[labeled_bs:].detach(), dim=1, keepdim=False)
                consistency_loss1 = 0.5*(ce_loss(outputs_round3_1[labeled_bs:], pseudo_outputs2.long()) + dice_loss(outputs_round3_1[labeled_bs:], pseudo_outputs2, softmax=True))
                consistency_loss2_r = 0.5*(ce_loss(outputs_round3_2_r[labeled_bs:], pseudo_outputs2.long()) + dice_loss(outputs_round3_2_r[labeled_bs:], pseudo_outputs2, softmax=True)) + 0.5*(ce_loss(outputs_round3_2[labeled_bs:], pseudo_outputs2.long()) + dice_loss(outputs_round3_2[labeled_bs:], pseudo_outputs2, softmax=True))
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()

            loss3 = 0.5*(supervised_round3_loss1 + supervised_round3_loss1_r) + consistency_weight * consistency_loss1 + args.coe_PCR * consistency_loss2_r
            optimizer.zero_grad()
            loss3.backward()
            optimizer.step()

            del outputs_round3_1, outputs_round3_2, outputs_round3_2_r, image_batch, label_batch, outputs_round3_soft1, outputs_round3_soft2, outputs_round3_soft2_r
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            ...
            if iter_num > 0 and iter_num % 200 == 0:  
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i = test_single_volume(
                        sampled_batch["image"], sampled_batch["label"], model, classes=num_classes+1, patch_size=[args.img_size, args.img_size]) 
                    metric_list += np.array(metric_i)
                metric_list = metric_list / len(db_val)
                for class_i in range(num_classes):   
                    writer.add_scalar('info/val_{}_dice'.format(class_i+1),
